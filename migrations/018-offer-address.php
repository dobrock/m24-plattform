<?php
/**
 * Migration 018 — Rechnungs- + Lieferadresse an m24_offers (Angebot = Auftrag).
 *
 * Teil 4 der Angebots-Annahme: bei der verbindlichen Annahme erfasste Adresse wird als eigene Spalten am
 * Auftrag gespeichert (Versand-Quelle). Rein additiv, idempotent via dbDelta (fügt fehlende Spalten hinzu,
 * bestehende Angebote bekommen NULL). Voller Schema-Restate ist dbDelta-Pflicht (es diff-t auf die Zielform).
 */
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
function m24_migration_018() {
    global $wpdb;
    ...
    // Verifizieren, dass eine neue Spalte tatsächlich existiert (idempotent, kein Fehler bei erneutem Lauf).
    $col = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM {$table} LIKE %s", 'bill_strasse' ) ); // phpcs:ignore WordPress.DB.PreparedSQL
    if ( 'bill_strasse' !== $col ) {
        error_log( 'M24 Plattform Migration 018: Spalte bill_strasse fehlt nach dbDelta an ' . $table );
        return false;
    }
    return true;
}
