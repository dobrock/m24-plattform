<?php
/**
 * Migration 021 — Ablauf-Reminder-Flag (m24_offers).
 *
 * Eine additive Spalte reminder_sent_at (DATETIME NULL): Zeitpunkt der einmaligen Erinnerungs-Mail ~2 Tage
 * vor Fristende. NULL = noch kein Reminder → der Cron (M24_Offers::remind_due) sendet einmalig und setzt das
 * Flag atomar. Rein additiv, idempotent via dbDelta; voller Schema-Restate ist dbDelta-Pflicht.
 */
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
function m24_migration_021() {
    global $wpdb;
    ...
    $col = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM {$table} LIKE %s", 'reminder_sent_at' ) ); // phpcs:ignore WordPress.DB.PreparedSQL
    if ( 'reminder_sent_at' !== $col ) {
        error_log( 'M24 Plattform Migration 021: Spalte reminder_sent_at fehlt nach dbDelta an ' . $table );
        return false;
    }
    return true;
}
