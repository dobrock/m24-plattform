<?php
/**
 * Migration 031 — Abweichung gegenüber der versendeten Fassung (Drift-Marker).
 *
 * Seit 0.11.484 legt sync_supersede keinen Nachfolger mehr an (Notschalter). Ohne Ersatz-Angebot
 * gäbe es aber auch kein sichtbares Zeichen mehr, dass ein bereits versendetes Angebot inzwischen
 * inhaltlich abweicht — genau das darf nicht passieren. Statt einer neuen Nummer merkt sich das
 * Angebot die Abweichung an sich selbst:
 *   offer_drift        TINYINT(1) — 1 = weicht von der versendeten Fassung ab
 *   offer_drift_at     DATETIME   — UTC, wann die Abweichung erkannt wurde
 *   offer_drift_fields TEXT       — JSON-Liste der abweichenden Felder (fürs Log und die Karte)
 *
 * Angebote liegen in einer eigenen Tabelle, nicht als Post — deshalb Spalten und kein Postmeta.
 *
 * Rein additiv, idempotent via dbDelta; voller Schema-Restate ist dbDelta-Pflicht (diff-t auf die
 * Zielform). Die Spaltenliste stammt unverändert aus Migration 030 plus den drei neuen Feldern.
 */
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
function m24_migration_031() {
    global $wpdb;
    ...
    foreach ( array( 'offer_drift', 'offer_drift_at', 'offer_drift_fields' ) as $col ) {
        $have = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM {$table} LIKE %s", $col ) ); // phpcs:ignore WordPress.DB.PreparedSQL
        if ( $col !== $have ) {
            error_log( 'M24 Plattform Migration 031: Spalte ' . $col . ' fehlt nach dbDelta an ' . $table );
            return false;
        }
    }

    if ( class_exists( 'M24_Logger' ) ) {
        M24_Logger::info( 'migration', 'Migration 031: Drift-Marker angelegt', array() );
    }
    return true;
}
