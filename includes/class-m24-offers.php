...
class M24_Offers {
	...
	public static function render_admin_list() {
		...
		if ( isset( $_GET['rec'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( '' !== $rec_txt ) {
				$rec_err = ( false !== strpos( $rec_txt, '✗' ) );
				echo '<div class="notice notice-' . ( $rec_err ? 'warning' : 'info' ) . ' is-dismissible"><p><strong>Desk-Abgleich:</strong> ' . esc_html( $rec_txt ) . '</p></div>';
			}
		}
		...
		if ( 'preview' === ( isset( $_GET['backfill'] ) ? sanitize_key( wp_unslash( $_GET['backfill'] ) ) : '' ) && class_exists( 'M24_Desk_Inbound' ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			...
			if ( empty( $cands ) ) {
				...
				$cnt = count( $cands );
				echo '<p>Folgende <b>' . (int) $cnt . '</b> gesyncte' . ( 1 === $cnt ? 's' : '' ) . ' Angebot' . ( 1 === $cnt ? '' : 'e' ) . ' werden mit <b>echtem Datum</b>, <b>Status</b>, <b>Positionen (mit Preis)</b> und <b>Netto/Brutto (inkl. korrektem MwSt-Satz)</b> neu materialisiert. Das <b>Datum</b> kommt – wo verfügbar – direkt aus dem Desk (<code>GET /api/orders</code>); nur wo der Desk keins liefert, greift der <b>Monatsanfang</b> aus der Auftragsnummer:</p>';
				if ( 0 === (int) $bres['dates_pulled'] ) { echo '<p style="color:#9a6b25;font-size:12.5px;margin:0 0 8px">Hinweis: Der Desk lieferte kein Angebotsdatum (nicht erreichbar oder Feld unbekannt) → für alle Zeilen greift der Monatsanfall-Fallback. Prüfe die Desk-Verbindung bzw. das GET-Payload-Datumsfeld.</p>'; }
				if ( $skipped > 0 ) { echo '<p style="color:#9a6b25;font-size:12.5px;margin:0 0 8px">' . (int) $skipped . ' Zeile' . ( 1 === $skipped ? '' : 'n' ) . ' mit nicht ermittelbarem Steuerfall (z. B. OSS ohne Desk-Brutto) werden bei den <b>Summen</b> bewusst übersprungen und bleiben auf 0,00 € (Status/Datum werden trotzdem gesetzt), bis ein Desk-Re-Push amt/vat_mode liefert.</p>'; }
		...
		// Vorschau-Panel „Desk-Aufträge (10 Tage) spiegeln": zieht GET /api/orders, listet die noch NICHT in WP
		// gespiegelten Aufträge der letzten 10 Tage. KEIN Schreiben bis zum zweiten Klick (Sicherheits-Gate).
		if ( 'preview' === ( isset( $_GET['mirror'] ) ? sanitize_key( wp_unslash( $_GET['mirror'] ) ) : '' ) && class_exists( 'M24_Desk_Inbound' ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			$mres   = M24_Desk_Inbound::find_mirror_candidates( 10 );
			$mnew   = $mres['new'];
			$cancel = esc_url( add_query_arg( array( 'page' => $page ), admin_url( 'admin.php' ) ) );
			echo '<div style="max-width:1000px;background:#fff;border:1.5px solid #0e447e;border-radius:12px;padding:18px 20px;margin:0 0 18px">';
			echo '<h2 style="margin:0 0 8px;font-size:16px">Desk-Aufträge der letzten 10 Tage spiegeln</h2>';
			echo '<p style="color:#6b7280;font-size:12.5px;margin:0 0 8px">Desk-Pull: <b>' . (int) $mres['pulled'] . '</b> Aufträge geladen · <b>' . (int) $mres['existing'] . '</b> bereits in WP (übersprungen) · <b>' . (int) $mres['undated'] . '</b> ohne Datum (übersprungen).</p>';
			if ( 0 === (int) $mres['pulled'] ) {
				echo '<p style="color:#9a3412">Der Desk lieferte <b>keine</b> Aufträge (nicht erreichbar oder unerwartetes Antwortformat). Bitte Desk-Verbindung prüfen.</p>';
			}
			if ( ! empty( $mres['keys'] ) ) {
				echo '<p style="color:#6b7280;font-size:11.5px;margin:0 0 8px">Diagnose (Feldnamen der ersten Desk-Zeile — zum Mapping-Abgleich): <code>' . esc_html( implode( ', ', array_map( 'strval', (array) $mres['keys'] ) ) ) . '</code></p>';
			}
			if ( empty( $mnew ) ) {
				echo '<p>Keine neuen Desk-Aufträge der letzten 10 Tage zum Spiegeln.</p><a class="chip" href="' . $cancel . '">Zurück</a>';
			} else {
				$mcnt = count( $mnew );
				echo '<p><b>Bitte prüfen</b>, bevor angelegt wird: Zeigen Kunde/Datum/Status/Betrag plausible Werte? Wenn ja → anlegen. Wenn leer/0 → Feld-Mapping stimmt nicht (Diagnose oben teilen, dann mappe ich exakt).</p>';
				echo '<table class="widefat striped"><thead><tr><th>Nr.</th><th>Kunde</th><th>Datum</th><th>Status</th><th>Brutto</th></tr></thead><tbody>';
				foreach ( $mnew as $c ) {
					$p    = $c['preview'];
					$stk  = (string) ( $p['status'] ?? '' );
					$slbl = isset( $badges[ $stk ] ) ? $badges[ $stk ][0] : ucfirst( $stk );
					echo '<tr><td>' . esc_html( (string) $p['order_num'] ) . '</td><td>' . esc_html( (string) $p['name'] ) . '</td><td>' . esc_html( (string) $p['date'] ) . '</td><td>' . esc_html( $slbl ) . '</td><td>' . esc_html( number_format( (float) $p['gross'], 2, ',', '.' ) ) . ' €</td></tr>';
				}
				echo '</tbody></table>';
				echo '<form method="post" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" style="display:flex;gap:10px;align-items:center;margin-top:8px">';
				echo '<input type="hidden" name="action" value="m24_mirror_backfill">';
				wp_nonce_field( 'm24_mirror_backfill' );
				echo '<button type="submit" class="button button-primary">Jetzt spiegeln (' . (int) $mcnt . ')</button>';
				echo '<a class="chip" href="' . $cancel . '">Abbrechen</a>';
				echo '</form>';
			}
			echo '</div>';
