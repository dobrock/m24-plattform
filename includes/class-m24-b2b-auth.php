...
class M24_B2B_Auth {
    ...
    private static function form_css(): string {
        return '<style>'
            ...
            // Selects exakt auf Input-Höhe zwingen (Theme gibt ihnen sonst ~63px): fixe 56er-Box, vertikal zentriert.
            . '.m24b2b-card select{height:56px!important;min-height:56px!important;padding-top:0!important;padding-bottom:0!important}'
    ...
    /**
     * DIE kanonische Mail-Shell (siehe m24_mail_shell / docs/MAIL-SHELL.md). Kein eigenes Layout mehr —
     * zentral in m24_mail_shell() abgelegt, hier nur delegieren (Header/Footer identisch zu allen Mails).
     */
    private static function mail_html( string $headline, string $inner, string $lang = 'de' ): string {
        if ( function_exists( 'm24_mail_shell' ) ) {
            return m24_mail_shell( $headline, $inner, array( 'lang' => $lang ) );
        }
