...
class M24FZ_SEO {
	...
	/** Zwischengröße 1200px (Karten/Templates). Fuer og:image NICHT mehr verwendet — dort
	 *  erzeugt M24_Share_Image ein echtes JPEG (m24_og laeuft durch die WebP-Konvertierung). */
	...
	private static function og_image( $id ) {
		...
		if ( $att ) {
			// Gemeinsame Quelle: garantiert JPEG (nie WebP — WhatsApp rendert das nicht) und
			// umgeht Photon. Siehe includes/class-m24-share-image.php.
			$src = M24_Share_Image::for_attachment( $att );
			if ( $src ) {
		...
		// Auch der Notnagel muss teilbar sein: WebP/AVIF/leer → festes Marken-JPEG.
		$logo = M24_Share_Image::safe_url_or_default( $logo );
		return ( '' !== $logo ) ? array( 'url' => $logo, 'w' => 0, 'h' => 0, 'alt' => get_bloginfo( 'name' ) ) : null;
	}

	/* Die frueheren Helfer share_image()/generate_og_size()/size_url()/og_quality() sind
	 * entfallen — sie lieferten die WP-Zwischengroesse m24_og, die auf dieser Installation
	 * per `image_editor_output_format` zu WebP konvertiert wird (WhatsApp zeigte deshalb
	 * kein Vorschaubild). Zustaendig ist jetzt M24_Share_Image (eine Quelle fuer alle
	 * OG-Module: Beitraege, Fahrzeuge, Teile). */
